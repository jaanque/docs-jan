...
<div class="eng-card group p-6! h-full">
  <div class="flex items-start justify-between mb-6">
    <div class="flex items-center gap-6">
      <div class="w-20 h-20 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center text-sm font-black text-slate-500 shadow-sm transition-premium overflow-hidden shrink-0">
        {#if project.logo}
          <img src={project.logo} alt={project.id} class="w-full h-full object-contain p-1.5" />
        {:else}
          {project.id}
        {/if}
      </div>
    ...
    <div class="flex items-center gap-2 shrink-0">
       ...
       <button class="p-1.5 text-slate-300 hover:text-brand-accent transition-colors" title="Pin repository">
  ...
  <div class="space-y-4 flex-1">
    ...
    <div class="flex items-center gap-3">
       <div class="flex items-center gap-1.5">
         <span class="w-2 h-2 rounded-full" style="background-color: {project.langColor}"></span>
  ...
  <div class="mt-6 flex items-center justify-between pt-5 border-t border-slate-100/60 transition-colors">
    <div class="flex items-center gap-5 text-slate-400">
      <div class="flex items-center gap-1.5 hover:text-brand-primary cursor-pointer transition-colors" title="Stars">
        ...
        <span class="text-[12px] font-black">{project.stars}{project.stars > 100 ? 'k' : ''}</span>
      </div>
      <div class="flex items-center gap-1.5 hover:text-brand-primary cursor-pointer transition-colors" title="Forks">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
        <span class="text-[12px] font-black">{project.forks}</span>
      </div>
    </div>
    
    <button class="text-[11px] font-black text-brand-accent transition-colors flex items-center gap-1 hover:underline">
      Browse Docs
