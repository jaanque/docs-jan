<script lang="ts">
  ...
  async function handleRegister() {
    ...
    if (error) {
      ...
      message = '¡Registro completado! Revisa tu email para confirmar.';
    }
    loading = false;
  }
  ...
  <div class="w-full max-w-[400px] flex flex-col items-center">
    ...
    <div class="text-center mb-8 w-full">
      ...
      <p class="text-[15px] text-slate-500 font-medium">
        ¿Ya tienes cuenta? 
        <a href={resolve('/auth/login')} class="text-brand-accent hover:text-brand-accent-hover transition-colors inline-flex items-center gap-1 font-bold">
          Inicia sesión <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
        </a>
      </p>
    </div>
    ...
    <div class="w-full space-y-6">
      ...
      <form onsubmit={handleRegister} class="space-y-4">
        <div class="space-y-2">
          <label for="email" class="text-[14px] font-bold text-slate-900">Email de ingeniería</label>
        ...
        <div class="space-y-2">
          <label for="password" class="text-[14px] font-bold text-slate-900">Crea una contraseña</label>
          <div class="relative group">
            <div class="absolute inset-y-0 left-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-brand-accent transition-colors">
        ...
        {#if message}
          <p class="text-[13px] font-bold text-center {message.includes('completado') ? 'text-brand-success' : 'text-red-500'}">
            {message}
          </p>
        {/if}
        ...
        >
          {loading ? 'Preparando...' : 'Comenzar ahora'}
        </button>
      </form>
      ...
      <p class="text-center text-[13px] text-slate-500 font-medium leading-relaxed px-4">
        Al registrarte, declaras que has leído y aceptas nuestra 
        ...
        y los 
        <button type="button" class="text-slate-900 underline hover:text-brand-accent bg-transparent border-none p-0 cursor-pointer">Términos del Servicio</button>.
      </p>
    </div>
  </div>
</div>
